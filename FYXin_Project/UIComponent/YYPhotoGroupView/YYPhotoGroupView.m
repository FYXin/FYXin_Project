...
static const CFTimeInterval kPresentTimeDuration = 0.35;
static const CFTimeInterval kDismissTimeDuration = 0.3;
...
- (void)setItem:(YYPhotoGroupItem *)item {
    ...
    @weakify(self);
    [_imageView sd_setImageWithURL:item.largeImageURL placeholderImage:item.thumbImage options:kNilOptions progress:^(NSInteger receivedSize, NSInteger expectedSize) {
        @strongify(self);
        if (!self) return;
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    if (!item.thumbClippedToTop) {
        if ([[SDImageCache sharedImageCache] imageFromDiskCacheForKey:item.largeImageURL.absoluteString]) {
              cell.item = item;
        }
    }
    ...
    if (item.thumbClippedToTop) {
        ...
        float oneTime = animated ? kPresentTimeDuration : 0;
        [UIView animateWithDuration:oneTime delay:0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
        ...
        float oneTime = animated ? kPresentTimeDuration : 0;
        [UIView animateWithDuration:kPresentTimeDuration delay:0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
            _blurBackground.alpha = 1;
        }completion:NULL];
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    if (fromView == nil) {
        ...
        [UIView animateWithDuration:animated ? kDismissTimeDuration : 0 delay:0 options:UIViewAnimationOptionBeginFromCurrentState|UIViewAnimationOptionCurveEaseOut animations:^{
            self.alpha = 0.0;
    ...
    if (_fromItemIndex != currentPage) {
        ...
        [_background.layer addFadeAnimationWithDuration:kDismissTimeDuration curve:UIViewAnimationCurveEaseOut];
    } else {
        _background.image = _snapshorImageHideFromView;
    }
...
//    }
    ...
    [UIView animateWithDuration:animated ? kDismissTimeDuration : 0 delay:0 options:UIViewAnimationOptionBeginFromCurrentState|UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        [UIView animateWithDuration:animated ? kDismissTimeDuration/2 : 0 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
            self.alpha = 0;
        } completion:^(BOOL finished) {
...
- (void)cancelAllImageLoad {
    [_cells enumerateObjectsUsingBlock:^(YYPhotoGroupCell *cell, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)showHUD:(NSString *)msg {
    ...
    CGSize size = [self sizeForFont:font string:msg size:CGSizeMake(200, 200) mode:NSLineBreakByCharWrapping];
...
- (CGSize)sizeForFont:(UIFont *)font string:(NSString *)str size:(CGSize)size mode:(NSLineBreakMode)lineBreakMode {
    CGSize result;
    if (!font) font = [UIFont systemFontOfSize:12];
    if ([str respondsToSelector:@selector(boundingRectWithSize:options:context:)]) {
        ...
        CGRect rect = [str boundingRectWithSize:size options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attr context:NULL];
        result = rect.size;
    } else {
    ...
    switch (g.state) {
        ...
        case UIGestureRecognizerStateEnded: {
            ...
            if (fabs(v.y) > 1000 || fabs(deltaY) > 120) {
                ...
                [UIView animateWithDuration:0.4 delay:0 usingSpringWithDamping:0.9 initialSpringVelocity:v.y / 1000 options:  UIViewAnimationOptionAllowUserInteraction | UIViewAnimationOptionBeginFromCurrentState animations:^{
                    _scrollView.top = 0;
                    _blurBackground.alpha = 1;
                    _pager.alpha = 1;
                } completion:^(BOOL finished) {
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo{
    ...
    UIActivityIndicatorView *indicatorView = (UIActivityIndicatorView *)[self viewWithTag:'indi'];
